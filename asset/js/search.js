document.addEventListener("DOMContentLoaded", function () {
    ...
    function displayResults(results) {
        ...
        results.forEach((item) => {
            ...
            if (item.top_time) displayText += ` ${item.top_time}`;
            if (item.top_time_error != "-") displayText += `±${item.top_time_error}`;
            if (item.bottom_time) displayText += ` 至 ${item.bottom_time}`;
            if (item.bottom_time_error != "-")
                displayText += `±${item.bottom_time_error} <br>`;
